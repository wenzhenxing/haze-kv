...
func NewKeyValue(k string, v string) KeyValue {
	return KeyValue{k, v}
}
...
func MakeMapReduce(nmap int, nreduce int, file string, master string) *MapReduce {
	//mr is a MapReduce obj.
	mr := InitMapReduce(nmap, nreduce, file, master)
	Logger.Printf("MapReduce Info--------------\n")
	Logger.Printf("nmap:%d\n", mr.nMap)
	Logger.Printf("nreduce:%d\n", mr.nReduce)
	Logger.Printf("MasterAddress:%s\n", mr.MasterAddress)
...
/**
 * exposed func
 * <tt>func (t *T) MethodName(argType T1, replyType *T2) error<tt>
 * @chuan 2015/07/27
 */
func (mr *MapReduce) Register(args *RegisterArgs, res *RegisterReply) error {
	Logger.Printf("Register: worker %s\n", args.Worker)
	mr.registerChannel <- args.Worker //mutex
	res.OK = true
	return nil
}
...
func (mr *MapReduce) Shutdown(args *ShutdownArgs, res *ShutdownReply) error {
	...
	mr.alive = false //terminal the infinite loop
...
func (mr *MapReduce) StartRegistrationServer() {
	...
	go func() {
		for mr.alive {
			conn, err := mr.l.Accept() //l:net.Listener block utill client Dail
			//add := conn.LocalAddr()
			Logger.Printf("accepted %T from worker\n", conn)
			if err == nil {
				go func() {
					Logger.Printf("worker connected to master\n")
					rpcs.ServeConn(conn) //block utill client Call
					conn.Close()
				}()
			} else {
...
func DoMap(JobNumber int, fileName string, nreduce int, Map func(string) *list.List) {
	...
	size := fi.Size()
	Logger.Printf("DoMap: read split %s %d\n", name, size)
	b := make([]byte, size)
	_, err = file.Read(b)
	if err != nil {
		log.Fatal("DoMap: ", err)
	...
	for r := 0; r < nreduce; r++ {
		...
		for e := res.Front(); e != nil; e = e.Next() {
			kv := e.Value.(KeyValue)
			//use %nreduce to divide into nreduce part
...
func (mr *MapReduce) Merge() {
	Logger.Printf("Merge phase\n")
	kvs := make(map[string]string)
	for i := 0; i < mr.nReduce; i++ {
		p := MergeName(mr.file, i)
		Logger.Printf("Merge: read %s\n", p)
		file, err := os.Open(p)
		if err != nil {
			log.Fatal("Merge: ", err)
		}
		...
		for {
			...
			if v, exsit := kvs[kv.Key]; exsit != false {
				a, _ := strconv.Atoi(v)
				b, _ := strconv.Atoi(kv.Value)
				vv := a + b
				kvs[kv.Key] = strconv.Itoa(vv)
			} else {
				kvs[kv.Key] = kv.Value
			}
		}
...
func (mr *MapReduce) Run() {
	Logger.Printf("Run mapreduce job %s %s\n", mr.MasterAddress, mr.file)
