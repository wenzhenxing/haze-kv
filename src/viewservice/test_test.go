...
func Test1(t *testing.T) {
	...
	runtime.GOMAXPROCS(4) //set the max aviliable CPU
	...
	{
		ck1.Ping(2) //after ping(2), new the viewserver want 3
		vx, _ := ck2.Ping(2)
		//this lead to the viewserver loss ck1
		for i := 0; i < DeadPings*2; i++ {
	...
	{
		vx, _ := ck2.Get()
		ck2.Ping(vx.Viewnum) //ack
		//this loop will kill ck2
		for i := 0; i < DeadPings*2; i++ {
			ck3.Ping(0) //entry the backup queue
	...
	{
		vx, _ := ck1.Get()
		ck1.Ping(vx.Viewnum) //ack
		for i := 0; i < DeadPings*2; i++ {
			ck1.Ping(0) //this will make ck1 dead
			ck3.Ping(vx.Viewnum)
			v, _ := ck3.Get()
			if v.Primary != ck1.me {
				break
...

	{
		for i := 0; i < DeadPings*2; i++ {
			v, _ := ck1.Get()
			ck1.Ping(v.Viewnum)
			ck2.Ping(0)         //ck2 entry the views
			ck3.Ping(v.Viewnum) //ack
			time.Sleep(PingInterval)
		}
		for i := 0; i < DeadPings*2; i++ {
			ck2.Ping(0) //ck1 ck3 die
			time.Sleep(PingInterval)
		}
		vz, _ := ck2.Get()
